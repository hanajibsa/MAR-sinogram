...
if __name__=="__main__":
    ...
    parser.add_argument('--input_path', default='/mnt/aix21104/jw/data/syndiff/Train/sinogram/png', type=str)
    parser.add_argument('--save_path', default='/mnt/aix21104/jw/data/syndiff/Train/recon_img', type=str)
    # parser.add_argument('--anatomy', default='h', type=str)
    args = parser.parse_args()

    os.makedirs(args.save_path, exist_ok=True)
	
    inp_path = glob.glob(args.input_path+'/*.raw')
    # inp_path = os.listdir(args.input_path)

    for inp_file in inp_path:
        print(inp_file)
        save = os.path.basename(inp_file)
        save = save.replace('sino', 'img')

        if 'head' in inp_file:
            FOV = 220.16
        elif 'body' in inp_file:
            FOV = 400

        # inp_data = rawread(inp_file, [1000, 1, 900], 'float')
        inp_data = Image.open(inp_file)
        inp_data = image.resize((1000, 900), resample=Image.LANCZOS)
        inp_data = np.array(inp_data)
        print('resize 완료')
        # inp_data = np.array(inp_data).resize((900, 1000))


        cfg = AAPMRecon_init(inp_file, FOV)
        recon_img = AAPMRecon_main(cfg, inp_data)
        rawwrite(os.path.join(args.save_path, 'raw', save), recon_img)
        print(os.path.join(args.save_path, save))
        
        save_img = Image.fromarray(recon_img.astype('uint8'), mode='L')
        save_img.save(os.path.join(args.save_path, 'png', save[:-4]+'.png'))
        exit()
