...
class NCSNpp(nn.Module):
  ...
  def forward(self, x, time_cond, z):
    ...
    if self.embedding_type == 'fourier':
      ...
      temb = modules[m_idx](torch.log(used_sigmas)) #m_idx=0
      m_idx += 1
    ...
    if self.conditional:
      ...
      m_idx += 1
      temb = modules[m_idx](self.act(temb)) #m_idx=2
      m_idx += 1
    else:
      temb = None
    ...
    hs = [modules[m_idx](x)] #m_idx=3
    m_idx += 1
    for i_level in range(self.num_resolutions):
      # Residual blocks for this resolution
      for i_block in range(self.num_res_blocks):
        h = modules[m_idx](hs[-1], temb, zemb) #m_idx=4             ###ERROR
        m_idx += 1
      
        if h.shape[-1] in self.attn_resolutions:
          h = modules[m_idx](h) #m_idx=5
          m_idx += 1
