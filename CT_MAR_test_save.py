...
def sample_and_test(args):
    ...
    metal_dir = os.path.join(input_path, 'Baseline/sinogram')
    ...
    dataset = CustomDataset(metal_dir, nometal_dir, transform=transform, option='all')
    ...
    for iteration, batch in enumerate(data_loader): 
        ...
        fake_sample2_ori = fake_sample2.cpu().numpy().squeeze(0).squeeze(0).transpose(1,0)
        ...
        # if name == '04' or name =='05':
        #     FOV = 650
        # elif name == '06' or name =='07' or name =='08' or name =='09' or name =='10':
        #     FOV = 360
        # elif name == '18' or name =='19' or name =='20':
        #     FOV = 355
        # else:
        #     FOV = 500
        ...
        cfg = AAPMRecon_init('training_body_metalart_sino3_900x1000.png', FOV)
        recon_img = AAPMRecon_main(cfg, fake_sample2)

        # real_data = real_data.cpu().numpy()
        # fake_sample2_ori = fake_sample2_ori.reshape(512,512)
        ...
        # fake_sample2_save = (fake_sample2_save - fake_sample2_save.min()) / (fake_sample2_save.max() - fake_sample2_save.min()) * 255
        ...
        # save recon image 
        save_img = Image.fromarray(recon_img.astype('uint8'), mode='L')
        save_img.save(os.path.join('/mnt/aix21104/jw/data/phase3_test_data_1000/all/syndiff_1000/recon_img', 'png', 'syndiff_{}_{}.png'.format(option, name)))
        rawwrite(os.path.join('/mnt/aix21104/jw/data/phase3_test_data_1000/all/syndiff_1000/recon_img', 'raw', 'syndiff_{}_{}.raw'.format(option, name)), recon_img)
        
        # print(f'save syndiff_{option}_{name}')

        # save sinogram 
        save_sino = Image.fromarray(fake_sample2_ori.astype('uint8'), mode='L')
        save_sino.save(os.path.join('/mnt/aix21104/jw/data/phase3_test_data_1000/all/syndiff_1000/sinogram', 'png', 'syndiff_{}_{}.png'.format(option, name)))
        rawwrite(os.path.join('/mnt/aix21104/jw/data/phase3_test_data_1000/all/syndiff_1000/sinogram', 'raw', 'syndiff_{}_{}.raw'.format(option, name)), fake_sample2_ori)
        print(f'option: {option} / Fov: {FOV}')
        ...
        # save = 'recon_img_{}_{}.png'.format(option, name)
        # save_img = Image.fromarray(recon_img.astype('uint8'), mode='L')
        # save_img.save(os.path.join(args.save_path, save))
        ...
        # save_img.save(save_dir+'/epoch{}_recon_{}_{}.jpg'.format(epoch_chosen, name, int(label)))

        # loss1[0, iteration] = ssim(real_img, recon_img)
        # loss2[0, iteration] = psnr(real_img, recon_img)#.cpu().numpy()
        # print('SSIM:', loss1[0, iteration], 'PSNR:', loss2[0, iteration])
        

    # print('Final SSIM: ',np.nanmean(loss1))
    # np.save('{}/ssim.npy'.format(save_dir), loss1)

    # print('Final PSNR: ',np.nanmean(loss2))
    # np.save('{}/psnr.npy'.format(save_dir), loss2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', default='', help='path to output saves-ckpt')
    parser.add_argument('--save_path', default='', help='path to output saves-image')
